...
def main():
    ...
    ### PLOT THE PHASE PLOTS WITH APPROPRIATE CONDITIONS
    print(conf['Neuron'])
    ...
    print(conf['Trajectory'])        


    fig, ax = plt.subplots(
        nrows = int(conf['Figure']['n_rows']),
        ncols = int(conf['Figure']['n_cols']),
        figsize = np.array(conf['Figure']['figsize'].split(', '), dtype = np.int32)
    )
